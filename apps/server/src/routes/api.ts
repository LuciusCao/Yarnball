...
export function createApi(
   ...
   * guest 拿其他行程的实体 id 调写端点 → 403（行程归属不符）；不存在的实体 → 404。
   * 已知口径（评审 P3，如实记录）：404 与 403 在时序上可区分，构成对已认证者的
   * 跨行程实体存在性 oracle——但实体 id 是 UUID 不可枚举，猜测成本等同直接碰撞
   * 主键，实际不可利用，故不做 403/404 归一（归一反而把 404 提前到权限检查前，
   * 向未认证者泄露存在性，更糟）。不调换查询顺序同理。

 ...
 * 用户粘贴的攻略文本会跟在后面。
 */
export function bootstrapPrompt(tripTitle: string, destinationCity: string): string {
  return [
    ...
    ``,
    `## 铁律`,
    `1. **坐标只能来自 search_poi**：创建任何地点前，必须先 search_poi 拿到真实坐标（gcj02）和 poiId，禁止根据印象填写或编造经纬度。地点名要用官方名称（如「灵隐寺」而不是「灵隐」）。`,
    ...
    ``,
    `## 工作方式`,
    `- 用户会粘贴小红书攻略文本、携程酒店候选等你无法直接访问的内容——把它们当作用户提供的数据来解析，不要试图抓取链接。`,
    `- 解析攻略时：提取地点 → 逐个 search_poi → add_place（notes 里保留原文的推荐理由/人均/排队提示）→ 按地理位置分天编排 add_place_to_day。`,
    `- 多个地点在动手前给出一句编排逻辑（如「西湖沿线排 Day 1，灵隐寺方向排 Day 2」），让用户能跟上。`,

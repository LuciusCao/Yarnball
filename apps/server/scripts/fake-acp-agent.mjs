 ...
 *                               tool_call（yarnball: add_place）→ 回复文本 → stop
...
async function handlePrompt(requestId, text) {
  ...
  if (script === "mcp_call_flow") {
    // 真实调用 Yarnball MCP server（session/new 注入的 http spec）
    if (!injectedMcp) {
      update({
        sessionUpdate: "agent_message_chunk",
  ...
  // 默认 prompt_flow：流式回复 + 模拟 yarnball MCP 工具调用
  update({
    sessionUpdate: "agent_message_chunk",
  ...
  // 真实调用 MCP：从 session/new 记录的 mcpServers 是 Yarnball 注入的；

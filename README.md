...
传统攻略按内容组织（清单式的餐厅/景点/酒店列表），Odessey 按**空间和时间**组织：行程是地图上的节点 + 节点间的交通段，按天串联。你在右侧悬浮的 mac 毛玻璃面板里与**自己的 agent** 对话（ACP 接入，如 `kimi acp`、`gemini acp`、`claude-code-acp`），agent 通过 MCP 工具直接读写行程数据结构，全屏地图实时刷新。
...
- **ACP client**（`@agentclientprotocol/sdk`）：每会话一个 agent 子进程；permission 四层策略（Odessey 工具自动批准 → 只读 kind 自动批准 → allow-all → 停靠 UI 120s）；terminal 协议五方法（kimi 的 Bash/Grep 依赖）；bootstrap prompt（钉死「坐标必须来自 search_poi」纪律 + 海外英文搜索提示）；压缩转录回放恢复
...
打开 http://localhost:5173 → 创建行程（填 "Sydney" 或 "杭州"）→ 右侧面板选择 agent 连接 → 粘贴攻略文本。
